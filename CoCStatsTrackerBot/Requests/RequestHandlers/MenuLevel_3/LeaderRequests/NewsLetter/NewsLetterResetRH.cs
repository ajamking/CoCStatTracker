...
public class NewsLetterResetRH : BaseRequestHandler
{
    public NewsLetterResetRH()
    {
        Header = $"Рассылка {BeautyIcons.RedCircleEmoji}/{BeautyIcons.GreenCircleEmoji}";
    ...
    override public void Execute(BotUserRequestParameters parameters)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(parameters.LastClanTagToMerge))
            {
                ...
                UpdateDbCommandHandler.ResetClanRegularNewsLetter(clanDb.Tag, NewsLetterType.All);

                var tempAnswer = StylingHelper.MakeItStyled($"Операция успешна. Рассылка {(clanDb.RegularNewsLetterOn ? "вылючена." : "включена.")}\n", UiTextStyle.Subtitle);
