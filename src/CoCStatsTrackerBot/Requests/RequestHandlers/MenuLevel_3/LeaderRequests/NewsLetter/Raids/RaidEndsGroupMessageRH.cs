...
public class RaidEndsGroupMessageRH : BaseRequestHandler
{
    public RaidEndsGroupMessageRH()
    {
        Header = $"Конец рейдов {BeautyIcons.RedCircleEmoji}/{BeautyIcons.GreenCircleEmoji}";
    ...
    override public void Execute(BotUserRequestParameters parameters)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(parameters.LastClanTagToMerge))
            {
                ...
                UpdateDbCommandHandler.ResetClanRegularNewsLetter(clanDb.Tag, NewsLetterType.RaidEnd);

                var tempAnswer = StylingHelper.MakeItStyled($"Операция успешна. Рассылка о конце рейдов {(clanDb.RaidEndMessageOn ? "вылючена." : "включена.")}\n", UiTextStyle.Subtitle);
